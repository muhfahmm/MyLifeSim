...
// Rules:
// 1. Tepat 100 entri per file (400 per negara).
// 2. 0 Overlap (semua 400 nama 100% berbeda).
// 3. ZERO angka / nomor (no 1, 2, 3).
// 4. ZERO kode / tag (no -F, -M, -L, -LF).
// 5. Murni nama manusia asli dari wilayah tersebut.
...
const targetListFinal = ['vietnam', 'taiwan'];

for (const c of targetListFinal) {
  const baseDir = path.join('json', 'firstname_lastname', 'asia', c);
  const data = datasetsAll17[c];

  const cSet = new Set();
  function filterClean100(arr) {
    const res = [];
    for (let item of arr) {
  ...
  const mF = filterClean100(data.mF);
  const fF = filterClean100(data.fF);
  const mL = filterClean100(data.mL);
  const fL = filterClean100(data.fL);

  console.log(`${c} status: mF=${mF.length}, fF=${fF.length}, mL=${mL.length}, fL=${fL.length}`);
...
console.log('Finished updating Vietnam and Taiwan clean sets!');
