...
class ParentRemarriage {
  static void checkAndApplyRemarriage(Character character, Random random, List<String> events) {
    // Jeda minimum menikah lagi setelah cerai: 2-5 tahun
    const int minRemarryGap = 2;
    const int maxRemarryGap = 5;

    // Case 1: Ibu kandung hidup, tidak ada ayah kandung (atau wafat/cerai) dan belum punya ayah tiri
    if (character.motherName != null && !character.isMotherDeceased && character.stepFatherName == null) {
      ...
      if (character.isMotherDivorced) {
        // Harus lewati jeda 2-5 tahun setelah cerai sebelum boleh menikah lagi
        final int requiredGap = minRemarryGap + random.nextInt(maxRemarryGap - minRemarryGap + 1);
        if (character.motherDivorceYearsSince >= requiredGap) {
          // ibu menikah lagi 40% setelah masa jeda
          if (random.nextInt(100) < 40) {
            shouldRemarry = true;
          }
        }
      } else if (character.fatherName == null || character.isFatherDeceased) {
        // jika suami meninggal ibu menikah lagi 40% (langsung, tidak ada jeda cerai)
        if (random.nextInt(100) < 40) {
          shouldRemarry = true;
        }
      }
      ...
      if (shouldRemarry) {
        ...
        character.motherDivorceYearsSince = 0; // reset counter setelah menikah
    ...
    // Case 2: Ayah kandung hidup, tidak ada ibu tiri, belum dibujuk tidak menikah lagi
    if (character.fatherName != null &&
        !character.isFatherDeceased &&
        character.stepMotherName == null &&
        !character.isFatherPersuadedNotToRemarry) {
      bool shouldRemarry = false;
      final bool isFemaleUserWithFather = character.gender.toLowerCase() == 'perempuan' &&
          character.custodyParent == 'Ayah';

      if (character.isFatherDivorced) {
        // Harus lewati jeda 2-5 tahun setelah cerai sebelum boleh menikah lagi
        final int requiredGap = minRemarryGap + random.nextInt(maxRemarryGap - minRemarryGap + 1);
        if (character.fatherDivorceYearsSince >= requiredGap) {
        ...
        // jika istri meninggal ayah menikah lagi 40% (langsung, tidak ada jeda cerai)
      ...
      if (shouldRemarry) {
        ...
        character.fatherDivorceYearsSince = 0; // reset counter setelah menikah
