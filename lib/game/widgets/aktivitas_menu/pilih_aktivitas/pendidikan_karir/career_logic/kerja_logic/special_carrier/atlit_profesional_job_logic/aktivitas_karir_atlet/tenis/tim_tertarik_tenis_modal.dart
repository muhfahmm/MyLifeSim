...
import 'tenis_logic/gaji_pemain_tenis.dart';
import 'tenis_logic/logika_pemain_tenis.dart';

class TimTertarikTenisModal {
  static void show({
...
class _TimTertarikDialogState extends State<_TimTertarikDialog> {
  ...
  void _generateInterestedTeams() {
    final String currentJob = widget.character.jobName ?? 'Petenis Utama - Klub Tenis';
    String currentTeamName = '';
    String currentPosTitle = 'Petenis Utama';
    ...
    final rand = Random();
    final allTeams = TimOlahragaDatabase.getTeamsBySport('Tenis');
    final availableTeams = allTeams.isNotEmpty ? allTeams : [
      {'name': 'Klub Tenis Utama'},
      {'name': 'Klub Tenis Garuda'},
      {'name': 'Klub Tenis Nusantara'},
    ];
    ...
    _interestedTeams = selected.map((t) {
      final int offeredSalary = GajiPemainTenisLogic.hitungGajiBerdasarkanUsia(
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      title: Row(
        children: [
          Icon(Icons.swap_horiz, color: Colors.lightGreen, size: 26),
          const SizedBox(width: 8),
          Text('Klub Tenis Tertarik 🎾', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
        ],
      ),
      content: SizedBox(
