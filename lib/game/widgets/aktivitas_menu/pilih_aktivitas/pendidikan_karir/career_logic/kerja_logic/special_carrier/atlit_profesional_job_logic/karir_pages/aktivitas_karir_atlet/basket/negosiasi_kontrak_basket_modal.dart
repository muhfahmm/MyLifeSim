// lib/game/widgets/aktivitas_menu/pilih_aktivitas/pendidikan_karir/career_logic/kerja_logic/special_carrier/atlit_profesional_job_logic/karir_pages/aktivitas_karir_atlet/basket/negosiasi_kontrak_basket_modal.dart

import 'package:flutter/material.dart';
import 'dart:math';
import 'package:mylifesim/pilih_karakter/character.dart';
import 'package:mylifesim/pilih_karakter/settings/currency_settings.dart';
import 'basket_logic/logika_pemain_basket.dart';

class NegosiasiKontrakBasketModal {
  static void show({
...
class _NegosiasiDialogWidgetState extends State<_NegosiasiDialogWidget> {
  ...
  void initState() {
    super.initState();
    final validYears = LogikaPemainBasket.getOpsiDurasiKontrak(widget.character.age);
  ...
  void _submitNegotiation() {
    ...
    final String teamName = widget.offerData['teamName'] ?? 'Klub Basket';
    ...
    if (success) {
      ...
      final bool teamWithdraws = rand.nextInt(100) < 30;

      if (teamWithdraws) {
        final String notice = '❌ Negosiasi Gagal & Tawaran Dibatalkan!\n'
            'Manajemen $teamName tersinggung dengan tuntutan gajimu yang terlalu tinggi dan menarik kembali penawaran kontrak.';
        widget.character.inbox.add(notice);
        ...
        showDialog(
          ...
          builder: (ctx) => AlertDialog(
            ...
            title: const Row(
              children: [
                Icon(Icons.cancel, color: Colors.red, size: 26),
                SizedBox(width: 8),
                Text('Penawaran Dibatalkan! ❌', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
              ],
            ),
            content: Text(
              'Manajemen $teamName menolak tuntutan gajimu dan resmi membatalkan penawaran kontrak!',
              style: const TextStyle(fontSize: 14),
            ),
            actions: [
              TextButton(
        ...
        final int originalOfferSalary = widget.offerData['offeredSalary'] as int? ?? 10000;
        ...
        showDialog(
          ...
          builder: (ctx) => AlertDialog(
            ...
            content: Text(
              'Manajemen $teamName menolak tuntutanmu, namun kamu terpaksa menyetujui tawaran awal mereka ($originalOfferYears Tahun - ${CurrencySettings.format(originalOfferSalary)}/tahun).',
              style: const TextStyle(fontSize: 14),
            ),
            actions: [
              TextButton(
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      ...
      title: const Row(
        children: [
          Icon(Icons.handshake, color: Colors.orange, size: 26),
          SizedBox(width: 8),
          Text('Negosiasi Kontrak Basket 🏀', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
        ],
      ),
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                Text(CurrencySettings.format(_demandedSalary), style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.orange, fontSize: 14)),
              ],
            ),
            Slider(
