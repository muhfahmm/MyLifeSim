...
import 'action_menu/kontrak_manajemen/negotiate_contract_bulutangkis_action.dart';
import 'action_menu/kontrak_manajemen/consult_agent_bulutangkis_action.dart';

import 'action_menu/pertandingan_statistik/career_history_bulutangkis_modal.dart';

import 'action_menu/sosial_media_fans/press_conference_bulutangkis_action.dart';
import 'action_menu/sosial_media_fans/post_social_media_bulutangkis_action.dart';
...
class AtlitBulutangkisActivitiesPage extends StatefulWidget {
  ...
  const AtlitBulutangkisActivitiesPage({
    super.key,
  ...
  @override
  State<AtlitBulutangkisActivitiesPage> createState() => _AtlitBulutangkisActivitiesPageState();
}

class _AtlitBulutangkisActivitiesPageState extends State<AtlitBulutangkisActivitiesPage> {
  ...
  Widget build(BuildContext context) {
    ...
    for (var item in widget.character.athleteSeasonStats) {
      totalMainStat += (item['titles'] as num?)?.toInt() ?? 0;
    }
    final currentStats = widget.character.currentAthleteStats;
    if (currentStats != null) {
      totalMainStat += (currentStats['titles'] as num?)?.toInt() ?? 0;
    }

    return Scaffold(
      appBar: AppBar(
        title: Text('Menu Karir Atlet Bulutangkis 🏸'),
      ...
      body: ListView(
        ...
        children: [
          Card(
            elevation: 2,
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
            color: isDark ? Colors.grey.shade800 : Colors.teal.withValues(alpha: 0.1),
            child: Padding(
              padding: const EdgeInsets.all(16),
              child: Column(
                ...
                children: [
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            Text(
                              widget.character.jobName ?? 'Pebulutangkis Utama',
                              style: TextStyle(
                  ...
                  Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        _buildStatItem('Gelar Juara', '$totalMainStat 🏸', Icons.stars, Colors.amber, isDark),
          ...
          _buildCategoryHeader('LATIHAN & KONDISI FISIK', Icons.fitness_center, Colors.teal),
          _buildActionCard(
            title: 'Latihan Teknik & Ketajaman 🎯',
            subtitle: 'Melatih fokus dan teknik bertanding Bulutangkis',
            icon: Icons.sports,
            color: Colors.teal,
          ...
          _buildCategoryHeader('PERTANDINGAN & STATISTIK', Icons.stadium, Colors.teal),
          _buildActionCard(
            title: 'Riwayat Karir & Statistik 📜',
            subtitle: 'Lihat perolehan statistik pertandingan dan performa rating',
            icon: Icons.history,
            color: Colors.purple,
            onTap: () => CareerHistoryBulutangkisModal.show(
              context: context,
              character: widget.character,
            ),
            isDark: isDark,
          ...
          _buildActionCard(
            ...
            color: Colors.purple,
            onTap: () => NegotiateContractBulutangkisAction.execute(
              context: context,
              character: widget.character,
              onRefresh: _triggerRefresh,
              showResult: _showResult,
          ...
          _buildActionCard(
            ...
            onTap: () => ConsultAgentBulutangkisAction.execute(
              context: context,
              character: widget.character,
              onRefresh: _triggerRefresh,
            ),
          ...
          _buildActionCard(
            ...
            icon: Icons.mic,
            color: Colors.blue,
            onTap: () => PressConferenceBulutangkisAction.execute(
              context: context,
              character: widget.character,
              onRefresh: _triggerRefresh,
              showResult: _showResult,
          ...
          _buildActionCard(
            ...
            onTap: () => PostSocialMediaBulutangkisAction.execute(
              context: context,
              character: widget.character,
              onRefresh: _triggerRefresh,
              showResult: _showResult,
