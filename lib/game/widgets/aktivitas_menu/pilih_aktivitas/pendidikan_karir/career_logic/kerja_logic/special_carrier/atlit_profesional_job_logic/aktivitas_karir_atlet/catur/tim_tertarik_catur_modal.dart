...
import 'catur_logic/gaji_pemain_catur.dart';
import 'catur_logic/logika_pemain_catur.dart';

class TimTertarikCaturModal {
  static void show({
...
class _TimTertarikDialogState extends State<_TimTertarikDialog> {
  ...
  void _generateInterestedTeams() {
    final String currentJob = widget.character.jobName ?? 'Pecatur Utama - Klub Catur';
    String currentTeamName = '';
    String currentPosTitle = 'Pecatur Utama';
    ...
    final rand = Random();
    final allTeams = TimOlahragaDatabase.getTeamsBySport('Catur');
    final availableTeams = allTeams.isNotEmpty ? allTeams : [
      {'name': 'Klub Catur Utama'},
      {'name': 'Klub Catur Garuda'},
      {'name': 'Klub Catur Nusantara'},
    ];
    ...
    _interestedTeams = selected.map((t) {
      final int offeredSalary = GajiPemainCaturLogic.hitungGajiBerdasarkanUsia(
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      title: Row(
        children: [
          Icon(Icons.swap_horiz, color: Colors.deepPurple, size: 26),
          const SizedBox(width: 8),
          Text('Klub Catur Tertarik ♟️', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
        ],
      ),
      content: SizedBox(
