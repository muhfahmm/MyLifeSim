// lib/game/widgets/aktivitas_menu/pilih_aktivitas/pendidikan_karir/career_logic/kerja_logic/special_carrier/atlit_profesional_job_logic/karir_pages/aktivitas_karir_atlet/basket/tim_tertarik_basket_modal.dart
...
import 'basket_logic/gaji_pemain_basket.dart';
import 'basket_logic/logika_pemain_basket.dart';

class TimTertarikBasketModal {
  static void show({
...
class _TimTertarikDialogState extends State<_TimTertarikDialog> {
  ...
  void _generateInterestedTeams() {
    final String currentJob = widget.character.jobName ?? 'Point Guard - Klub Basket';
    String currentTeamName = '';
    String currentPosTitle = 'Point Guard';
    ...
    final rand = Random();
    final allTeams = TimOlahragaDatabase.getTeamsBySport('Basket');
    ...
    _interestedTeams = selected.map((t) {
      final int offeredSalary = GajiPemainBasketLogic.hitungGajiBerdasarkanUsia(
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      title: const Row(
        children: [
          Icon(Icons.swap_horiz, color: Colors.orange, size: 26),
          SizedBox(width: 8),
          Text('Klub Basket Tertarik 🏀', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
        ],
      ),
      content: SizedBox(
