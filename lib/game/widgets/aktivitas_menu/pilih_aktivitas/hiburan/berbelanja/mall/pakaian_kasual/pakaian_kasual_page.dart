// lib/game/widgets/aktivitas_menu/pilih_aktivitas/hiburan/berbelanja/mall/pakaian_kasual/pakaian_kasual_page.dart
import 'package:flutter/material.dart';
import 'package:mylifesim/pilih_karakter/character.dart';
import 'package:mylifesim/pilih_karakter/settings/currency_settings.dart';
import 'database_brand.dart';

class PakaianKasualPage extends StatefulWidget {
  final Character character;
  final VoidCallback onComplete;

  const PakaianKasualPage({
    super.key,
  ...
  @override
  State<PakaianKasualPage> createState() => _PakaianKasualPageState();
}

class _PakaianKasualPageState extends State<PakaianKasualPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final brands = PakaianKasualDatabase.brands;
    final owned = widget.character.ownedShopping.where((e) => e['type'] == 'pakaian_kasual').toList();

    return DefaultTabController(
      length: 2,
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Toko Pakaian Kasual 👕', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
                    ...
                    child: ListTile(
                      ...
                      onTap: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => _PakaianKasualProductsPage(
                              character: widget.character,
                              brandName: b,
                              products: PakaianKasualDatabase.products[b] ?? [],
                  ...
                  ? Center(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text('Belum ada koleksi pakaian kasual.', style: TextStyle(color: isDark ? Colors.white54 : Colors.grey)),
                        ],
                      ),
                    )
                  : ListView.builder(
...
class _PakaianKasualProductsPage extends StatefulWidget {
  final Character character;
  final String brandName;
  final List<Map<String, dynamic>> products;
  final VoidCallback onComplete;

  const _PakaianKasualProductsPage({
    required this.character,
  ...
  @override
  State<_PakaianKasualProductsPage> createState() => _PakaianKasualProductsPageState();
}

class _PakaianKasualProductsPageState extends State<_PakaianKasualProductsPage> {
  static String _fmt(int amount) => CurrencySettings.format(amount);
