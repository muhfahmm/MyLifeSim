// lib/game/widgets/aktivitas_menu/pilih_aktivitas/hiburan/berbelanja/toko_online/buku_pengetahuan/buku_pengetahuan_page.dart
import 'package:flutter/material.dart';
import 'package:mylifesim/pilih_karakter/character.dart';
import 'package:mylifesim/pilih_karakter/settings/currency_settings.dart';
import 'database_brand.dart';

class BukuPengetahuanPage extends StatefulWidget {
  final Character character;
  final VoidCallback onComplete;

  const BukuPengetahuanPage({
    super.key,
  ...
  @override
  State<BukuPengetahuanPage> createState() => _BukuPengetahuanPageState();
}

class _BukuPengetahuanPageState extends State<BukuPengetahuanPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final brands = BukuPengetahuanDatabase.brands;
    final owned = widget.character.ownedShopping.where((e) => e['type'] == 'buku_pengetahuan').toList();

    return DefaultTabController(
      length: 2,
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Toko Buku Pengetahuan 📚', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
                    ...
                    child: ListTile(
                      ...
                      onTap: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => _BukuPengetahuanProductsPage(
                              character: widget.character,
                              brandName: b,
                              products: BukuPengetahuanDatabase.products[b] ?? [],
                  ...
                  ? Center(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text('Belum ada koleksi buku pengetahuan.', style: TextStyle(color: isDark ? Colors.white54 : Colors.grey)),
                        ],
                      ),
                    )
                  : ListView.builder(
                      ...
                      itemBuilder: (context, i) {
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            ...
                            subtitle: Text('Penerbit: ${item['brand'] ?? '-'}', style: TextStyle(color: isDark ? Colors.white70 : Colors.black54)),
...
class _BukuPengetahuanProductsPage extends StatefulWidget {
  final Character character;
  final String brandName;
  final List<Map<String, dynamic>> products;
  final VoidCallback onComplete;

  const _BukuPengetahuanProductsPage({
    required this.character,
  ...
  @override
  State<_BukuPengetahuanProductsPage> createState() => _BukuPengetahuanProductsPageState();
}

class _BukuPengetahuanProductsPageState extends State<_BukuPengetahuanProductsPage> {
  static String _fmt(int amount) => CurrencySettings.format(amount);
                    ...
                    : (canAfford
                        ? () {
                            setState(() {
                              ...
                              if (p.containsKey('intelligence')) {
                                widget.character.intelligence = (widget.character.intelligence + (p['intelligence'] as int)).clamp(0, 100);
                              }
                              widget.character.ownedShopping.add({
                                'type': 'buku_pengetahuan',
                                'name': p['name'],
