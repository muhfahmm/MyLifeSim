...
class InvestasiItem extends StatelessWidget {
  ...
  const InvestasiItem({super.key, required this.age});
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      onTap: () {
        if (age < 15) {
          _showLockedDialog(context, 'Investasi', 15);
          return;
        }
        Navigator.pop(context);
        Navigator.push(context, MaterialPageRoute(builder: (context) => const _InvestasiPage()));
      },
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            Text(
              '\$0',
              style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: isUnlocked ? Colors.blue : Colors.grey),
            ),
...
// --- HALAMAN INVESTASI ---
class _InvestasiPage extends StatelessWidget {
  const _InvestasiPage();

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Investasi'), backgroundColor: Colors.blue),
      body: const Center(child: Text('Lihat portofolio investasi Anda.\n(Fitur dalam pengembangan)', textAlign: TextAlign.center, style: TextStyle(fontSize: 18))),
    );
  }
}